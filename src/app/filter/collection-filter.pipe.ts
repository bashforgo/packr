...
export class CollectionFilterPipe implements PipeTransform {
  static predicates: Dictionary<Predicate> = {
    'binary/rarity': (data: ShortRarity, card: Card, coll: Collection, keywords: Dictionary<SearchTerm[]>) => {
      return data === Rarity.short(card.rarity);
    },
    'binary/class': (data: CardClass, card: Card, coll: Collection, keywords: Dictionary<SearchTerm[]>) => {
    ...
    'binary/missing': (data, card: Card, coll: Collection, keywords: Dictionary<SearchTerm[]>) => {
      const count = CollectionFilterPipe.getCount(card, coll, 'binary/golden' in keywords);
      return count < Rarity.max(Rarity.short(card.rarity));
    },
    ...
    'ranged/mana': (data: Range, card: Card, coll: Collection, keywords: Dictionary<SearchTerm[]>) => {
